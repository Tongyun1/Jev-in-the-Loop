...
assert.deepEqual(happyPlan.counts, [4, 5], "the active happy scene lifts density without filling every beat");
assert.equal(happyPlan.profile, "joyful");
assert.equal(happyPlan.development, "sequence");
assert.deepEqual([0, 2, 4, 6].map((bar) => localScenePlan("平静", bar).counts), [[2, 2], [2, 2], [3, 2], [2, 1]]);
assert.ok([0, 2, 4, 6].every((bar) => localScenePlan("开心", bar).counts.every((count) => count >= 3 && count <= 6)));
...
assert.ok(calm.every((phrase) => phrase.bars.every((bar) => bar.events.length <= 3)));
assert.ok(happy.every((phrase) => phrase.bars.every((bar) => bar.events.length >= 3)));
